...
document.getElementById('btn').addEventListener('click', function(){
    document.getElementById('result').innerHTML = `<img src="https://petrobras.azurewebsites.net/img/ajax-loader.gif">`
    const gt = document.getElementById('titulo').value

    const xhr = new XMLHttpRequest()
    xhr.open('GET', `https://www.googleapis.com/books/v1/volumes?q= ${gt} & key = AIzaSyB-SbDbkO_peKBPlIB_Hi3ucIrtQ9V2vf8` )
    ...
    if(this.status == 200 && this.readyState == 4){
        ...
        for(let  i=0; i<requis.items.length; i++ ){
           ...
           if(requis.items[i].volumeInfo.publishedDate == undefined){
            requis.items[i].volumeInfo.publishedDate ="Data de Publicação Não Encontrado!"
           }else if(requis.items[i].volumeInfo.publisher  == undefined){
            requis.items[i].volumeInfo.publisher = "Editora Não Encontrado"
           }else if(requis.items[i].volumeInfo.authors  == undefined){
            requis.items[i].volumeInfo.authors = "Autor Não Encontrado"
           }
    
           result.innerHTML += `<div id="resultBook"><p id="titl">${requis.items[i].volumeInfo.title }</p><p>${requis.items[i].volumeInfo.authors}</p>
           ...
           <a id="saibaMais" href= "${requis.items[i].volumeInfo.infoLink}">MAIS DETALHES</a></div>`
